<# 
    ...
    This merged script performs the following cleanup tasks:
      ...
      5. Removes OneStart startup items from Registry Run keys and Startup folders.
      6. Removes scheduled tasks related to OneStart—using both specific names and a generic search.
      6.1 Removes invalid RunOnce entries referencing OneStart.exe.
      7. Removes additional OneStart-related Registry keys.
...
# ------------------------------
# 5. Remove Startup Items (Registry & Startup Folders)
# ------------------------------
...
# ------------------------------
# (a) Remove scheduled tasks with specific names.
...
# (b) Additional generic removal: Remove any scheduled task with 'OneStart' in its name or whose action references OneStart.exe.
try {
    $genericTasksToRemove = Get-ScheduledTask | Where-Object {
        $_.TaskName -match "(?i)onestart" -or
        ($_.Actions | ForEach-Object { $_.Execute } | Where-Object { $_ -match "(?i)onestart\.exe" })
    }
    foreach ($task in $genericTasksToRemove) {
        Write-Output "Removing scheduled task: $($task.TaskName)"
        Unregister-ScheduledTask -TaskName $task.TaskName -Confirm:$false -ErrorAction SilentlyContinue
    }
}
catch {
    Write-Output "Error while removing generic scheduled tasks: $_"
}

# ------------------------------
# 6.1 Remove Invalid RunOnce Entries (Additional Startup Cleanup)
# ------------------------------
$runOncePaths = @(
...
foreach ($regPath in $runOncePaths) {
    if (Test-Path $regPath) {
        $entries = Get-ItemProperty -Path $regPath -ErrorAction SilentlyContinue
        foreach ($entry in $entries.PSObject.Properties) {
            if ($entry.Value -match "(?i)onestart\.exe") {
                Write-Output "Removing RunOnce entry '$($entry.Name)' from $regPath"
